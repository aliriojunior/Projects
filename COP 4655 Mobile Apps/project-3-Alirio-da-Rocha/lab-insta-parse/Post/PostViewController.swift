...
class PostViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @IBAction func onShareTapped(_ sender: Any) {
            ...
            // Ensure we have a picked image
            guard let image = pickedImage,
                  let imageData = image.jpegData(compressionQuality: 0.1) else {
                return
            }

            // Create ParseFile for image
        ...
        // Add location to the post if available
            ...
            post.save { [weak self] result in
                DispatchQueue.main.async {
                    ...
                    case .success(let post):
                        print("✅ Post Saved with location: \(post)")
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            ...
            locationManager.stopUpdatingLocation()  // Stop updating once location is fetched
        }
    }

    // Called if location authorization fails
